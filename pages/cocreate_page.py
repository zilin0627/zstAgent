...
def _inject_css():
    st.markdown(
        ...
        .gold-hr {
            height: 2px;
            border: none;
            background: linear-gradient(90deg, #c9a227, #e8d06a, #c9a227);
            margin: 4px 0 14px;
        }
        /* ── 配置卡片 ── */
        .config-card {
...
def _panel_header(title: str, subtitle: str = "", color: str = "#1e3a5f"):
    sub_html = f'<div class="panel-hd-sub">{subtitle}</div>' if subtitle else ""
    st.markdown(
        f'<div class="panel-hd" style="background:{color};">'
...
def _render_result_sections(result_text: str):
    ...
    for title, body in result_sections:
        st.markdown(
            f'<div class="result-section-title">{title}</div>',
            unsafe_allow_html=True,
        )
        st.write(body or "—")
...
def render_cocreate_page(
    *,
    ...
    st.caption("选纹样 → 结合扫图观察 → 生成可进入方案讨论的结构化设计提案")
    st.markdown('<hr class="gold-hr">', unsafe_allow_html=True)

    # ── 主工作区：配置（左）+ 预览（右） ──────────────────
    workspace_left, workspace_right = st.columns([1.1, 0.9], gap="large")

    with workspace_left:
        _panel_header("设置任务", "选择纹样、用途和风格", color="#1e3a5f")
        ...
        extra = st.text_area(
            "补充说明",
            value=st.session_state.get("cocreate_extra", ""),
            height=72,
            key="cocreate_extra",
            placeholder="场景要求、受众特点、特殊限制…（可留空）",
        )

        with st.expander("高级选项"):
            generation_mode = st.radio(
                "调用模式", GENERATION_OPTIONS, index=1,
                key="cocreate_generation_mode", horizontal=True,
            )
            allow_web = generation_mode == "Agent + 联网"

        generate = st.button("🎨 生成提案", key="cocreate_generate", use_container_width=True, type="primary")
    ...
    with workspace_right:
        _panel_header("本次输入预览", "确认知识依据与设计语境", color="#1a5c4f")
        ...
        kw = '、'.join(pattern_item.get('keywords', [])) or '未标注'
        st.markdown(f"- **视觉关键词**：{kw}")
        ...
        allow_web = generation_mode == "Agent + 联网"
        mode_tips = {
            "直连 RAG":   "直连本地知识库，速度快，适合快速出第一版。",
            "Agent":      "Agent 工作流 + 本地知识库，结构更完整。",
            "Agent + 联网": "Agent + 联网，本地资料不足时自动补充。",
        }
        st.caption(mode_tips.get(generation_mode, ""))
    ...
    with reference_left:
        _panel_header("图谱卡片参考", "先看图谱，再做提案，结果更稳", color="#2d4a1e")
        ...
        if pattern_item:
            ...
            st.markdown(f"**{pattern_name}** · {pattern_item.get('category', '')}")
    ...
    with reference_right:
        _panel_header("真实扫图参考", "把针脚、层次和边饰关系带回设计转化", color="#4a2a10")
    ...
    with result_left:
        _panel_header("提案结果区", "生成后可直接用于方案初稿、课堂讨论或展陈文案整理", color="#3d2a5c")

        if generate:
            related_scans = get_related_scan_items(pattern_name)
            cocreate_prompt = build_cocreate_query(
                pattern_name, theme, target, tone, extra,
                related_scans, allow_web, get_pattern_item,
            )
        ...
        if cocreate_result:
            ...
            st.text_area("提案全文", value=cocreate_result, height=200, key="cocreate_result_text")
            ...
            st.markdown(
                """
                <div style="text-align:center; padding:32px 16px; color:#8a7050;">
                    <div style="font-size:32px; margin-bottom:10px;">✏️</div>
                    <div style="font-size:13px; line-height:1.7;">
                        完成左侧配置后点击「生成提案」<br>
                        <span style="font-size:11px; color:#b0956e;">
                            支持：展陈文案 · 文创概念 · 课程提案 · 品牌策划
                        </span>
                    </div>
                </div>
                """,
    ...
    with result_right:
        _panel_header("使用建议", "", color="#555")
        st.markdown("- 先锁定一个核心纹样，避免主题过宽泛")
        st.markdown("- 先观察图谱和扫图，再写补充要求，结果更贴近侗绣语境")
        st.markdown("- 目标是比赛/课程/品牌，把使用场景写进补充要求")
        st.markdown("- **直连 RAG**：快速打底稿")
        st.markdown("- **Agent**：完整结构 + 仅本地资料")
        st.markdown("- **Agent + 联网**：需补充公开资料或复杂任务")

        st.divider()
        st.markdown("**可输出到哪些场景**")
